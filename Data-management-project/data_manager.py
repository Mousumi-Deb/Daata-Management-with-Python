...
class DataManager:
    ...
    def __update_nan_values(self):
        ...
        while iso_codes:
            ...
            for i in range(1, len(records)):
                ...
                self.__update_nan_value_fixed_row(
                    db_cursor, current_record[0], current_record[1], current_record[2], updated_record)
                previous_record = []
    ...
    def __get_country_based_rows(self, countries, iso_code):
        ...
        db_cursor = db_connection.cursor()
        db_cursor.execute(
            "select * from vaccine_info where country_id = ? order by date asc", [countries[iso_code]])
        records = db_cursor.fetchall()
        rows = []
        for row in records:
            rows.append(row)
    ...
    def get_column_data(self, country_name, column_name, limit):
        db_connection = sqlite3.connect(self._database_name)
        db_cursor = db_connection.cursor()
        db_cursor.execute("select date, "+column_name +
                          " from vaccine_info, countries WHERE countries.name = ? AND countries.id = vaccine_info.country_id LIMIT 0, ?", [country_name, limit])
        dates = []
        column_data = []
        records = db_cursor.fetchall()
        for row in records:
